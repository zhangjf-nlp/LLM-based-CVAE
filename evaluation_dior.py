...
def bleu(hyps: list, refs: list, ignore_indices=None, output_path=None) -> tuple:
    ...
    output_content = 'BLEU-1/2: {}/{}'.format(round(bleu_1, 4), round(bleu_2, 4))
...
def distinct(hyps: list, output_path=None) -> tuple:
    ...
    output_content = 'Distinct-1/2: {}/{}'.format(round(inter_dist1, 4), round(inter_dist2, 4))
...
def entropy(hyps: list) -> tuple:
    ...
    for n in [1, 2, 3]:
        entropy_all = []
        for text in hyps:
            substrings = [simplify_text(text)[i:i + n] for i in range(len(text) - 1)]
            substrings = Counter(substrings)
            normalizer = sum(substrings.values())
            probs = dict((k, float(v) / normalizer) for k, v in substrings.items())
            entropy = -np.sum([p * np.log2(p) for p in probs.values()])
            entropy_all.append(entropy)
        results.append(np.mean(entropy_all))

    output_content = 'Ent-1/2/3: {}/{}/{}'.format(round(results[0], 4), round(results[1], 4), round(results[2], 4))
    print('-------------- Entropy score --------------\n{}'.format(output_content))
    return results
